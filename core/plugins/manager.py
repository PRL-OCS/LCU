...
class PluginManager:
    ...
    def fetch_hardware_mapping(self):
        ...
        """
        print("[SYSTEM] Fetching hardware mapping from API...")
        try:
            ...
            print(f"[SYSTEM] Hardware mapping updated with {len(self.hardware_mapping)} relationships.")

        except Exception as e:
            print(f"[ERROR] Failed to fetch hardware mapping: {e}")
    ...
    def validate_belonging(self, telescope_id: str, instrument_name: str) -> bool:
        ...
        if not self.hardware_mapping:
            print("[WARNING] Hardware mapping is empty. Validation bypassed.")
        ...
        if assigned_telescope is None:
            print(f"[VALIDATION] Unknown instrument '{instrument_name}'.")
            return False
            
        if assigned_telescope != telescope_id:
            print(f"[VALIDATION] Mismatch! {instrument_name} belongs to {assigned_telescope}, not {telescope_id}.")
            return False
            
        return True

    def _discover_telescope_plugins(self):
        print(f"[SYSTEM] Discovering telescope plugins in {self.telescope_dir}...")
        if not os.path.exists(self.telescope_dir):
            print(f"[WARNING] Telescope plugin directory {self.telescope_dir} does not exist.")
            return

        TelescopePlugin.registry = {}
        self._load_modules_from_dir(self.telescope_dir)
        ...
        for class_name, plugin_class in TelescopePlugin.registry.items():
            try:
                ...
                print(f"[SYSTEM] Loaded telescope plugin: {instance.get_id()}")
            except Exception as e:
                print(f"[ERROR] Failed to instantiate telescope plugin {class_name}: {e}")

    def _discover_instrument_plugins(self):
        print(f"[SYSTEM] Discovering instrument plugins in {self.instrument_dir}...")
        if not os.path.exists(self.instrument_dir):
            print(f"[WARNING] Instrument plugin directory {self.instrument_dir} does not exist.")
            return

        InstrumentPlugin.registry = {}
        self._load_modules_from_dir(self.instrument_dir)
        ...
        for class_name, plugin_class in InstrumentPlugin.registry.items():
            try:
                ...
                print(f"[SYSTEM] Loaded instrument plugin: {instance.get_id()}")
            except Exception as e:
                print(f"[ERROR] Failed to instantiate instrument plugin {class_name}: {e}")

    def _load_modules_from_dir(self, directory: str):
                ...
                if filename.endswith(".py") and not filename.startswith("__"):
                    ...
                    if spec and spec.loader:
                        try:
                            ...
                            print(f"[ERROR] Failed to import module {file_path}: {e}")
