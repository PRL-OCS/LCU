"""
LISA Camera Instrument Plugin
Drives the Atik camera via the LISA SDK REST API server (sdk_api_server.py).
...
class LISACameraPlugin(InstrumentPlugin):
    """
    Concrete instrument plugin for the LISA / Atik CCD camera.

    Communicates with the REST API server (sdk_api_server.py) which in turn
    manages a persistent CLI subprocess that talks to the Atik SDK.

    Default instrument_name = "LISA" — this is the key that the
    ...
    def _startup_connect(self):
        ...
        try:
            ...
            if not cli_ok:
                logger.warning(
                    f"[{self.instrument_name}] API server reachable but CLI "
                    f"subprocess is NOT running."
                )
        except Exception:
            logger.warning(
                f"[{self.instrument_name}] API server not reachable at "
                f"{self.api_url}. Commands will fail until it is started."
            )
            return
        ...
        if resp and resp.get("success"):
            ...
            logger.error(
                f"[{self.instrument_name}] Camera connect failed: "
                f"{resp.get('error') if resp else 'no response'}"
            )
